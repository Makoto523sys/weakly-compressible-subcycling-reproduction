...
function poisson!(p,b,rx,ry,c)
    ...
    z=zero_mean!(r./diag); d=copy(z); rz=dot(r,z)
...
function step!(s::State,dt,c::Config; mode::Symbol=:subcycling)
    ...
    if mode==:subcycling
        ...
        elapsed=0.0; nsub=0
        while elapsed < dt
            subdt=min(dt-elapsed,stable_dt(work,c))
...
function source_digest()
    ...
    files=sort(filter(p->endswith(p,".jl"),[joinpath(d,f) for (d,_,fs) in walkdir(@__DIR__) for f in fs]))
    io=IOBuffer()
    for path in files
...
function run_bubble(c::Config; mode=:subcycling,factor=30.0,t_end=0.001,
                    output="results/bubble",snapshot_interval=0.001)
    ...
    meta=Dict{String,Any}("status"=>"running_unvalidated","julia_version"=>string(VERSION),
        ...
        "config"=>Dict(string(f)=>getfield(c,f) for f in fieldnames(Config)))
    function metadata!()
    ...
    metadata!()
    start=time_ns(); next_snapshot=snapshot_interval; snapshot=0
    try
