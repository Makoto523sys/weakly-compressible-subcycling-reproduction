...
function interface_geometry(phi,c)
    ...
    Threads.@threads :static for j in 1:c.ny
...
speed(s) = hypot(maximum(abs,s.uf),maximum(abs,s.vf))
...
function fluxes(s::State,c::Config)
    ...
    Threads.@threads :static for j in 1:c.ny+1
        @inbounds for i in 1:c.nx
        vel=s.vf[i,j]
...
function pressure_operator!(out,p,rx,ry,c)
    Threads.@threads :static for j in 1:c.ny
        @inbounds for i in 1:c.nx
            value=0.0
            i>1 && (value+=(p[i,j]-p[i-1,j])/(rx[i,j]*dx(c)^2))
            i<c.nx && (value-=(p[i+1,j]-p[i,j])/(rx[i+1,j]*dx(c)^2))
            j>1 && (value+=(p[i,j]-p[i,j-1])/(ry[i,j]*dy(c)^2))
            j<c.ny && (value-=(p[i,j+1]-p[i,j])/(ry[i,j+1]*dy(c)^2))
            out[i,j]=value
        end
    end
    out
...
function poisson!(p,b,rx,ry,c)
    ...
    diag=zeros(size(p))
    @inbounds for j in 1:c.ny,i in 2:c.nx
        a=1/(rx[i,j]*dx(c)^2); diag[i,j]+=a; diag[i-1,j]+=a
    end
    @inbounds for j in 2:c.ny,i in 1:c.nx
...
function write_vtk(path,s,c)
    open(path,"w") do io
        ...
        @inbounds for j in 1:c.ny,i in 1:c.nx
