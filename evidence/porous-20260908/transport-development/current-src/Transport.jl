...
Base.@kwdef struct TransportParameters
    ...
    inlet_u::Float64=.003
    inlet_v::Float64=0.0
    no_slip_outer_walls::Bool=true
    weno_epsilon::Float64=1e-12
end

@inline function transport_sample(q,i,j,kind,c)
    nx,ny=size(q)
    if c.no_slip_outer_walls && (j<1 || j>ny) && kind in (:u,:v)
        jj=j<1 ? min(1-j,ny) : max(2ny+1-j,1)
        return -transport_sample(q,i,jj,kind,c)
    end
    if i<1
...
"""
function transport_step!(phi,u,v,uf,vf,dt,g,cv,c::TransportParameters;integrated_force=nothing,kwargs...)
    ...
    if integrated_force!==nothing
        for k in eachindex(phi)
            a=cv.owner[k];a==0 && continue
            ru[a]+=integrated_force[1][k];rv[a]+=integrated_force[2][k]
        end
    end
