...
function extend_contact!(phi,distance,nxwall,nywall,dx,dy,theta;iterations=80,band=3max(dx,dy))
    ...
    nx,ny=size(phi);cotangent=cotd(theta);dtau=.5min(dx,dy);old=copy(phi)
    ghosts=[CartesianIndex(i,j) for j in 1:ny for i in 1:nx if -band<distance[i,j]<0]
    for _ in 1:iterations
        # Fluid samples are frozen throughout this extension call. Copy and
        # visit only the band that can change, preserving Jacobi iteration.
        for p in ghosts;old[p]=phi[p];end
        for p in ghosts
            i,j=Tuple(p)
