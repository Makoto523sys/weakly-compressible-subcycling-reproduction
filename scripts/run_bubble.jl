...
function main(args)
    if isempty(args) || args[1] in ("-h","--help")
        println("Usage: julia --project=. scripts/run_bubble.jl MODE NX FACTOR T_END OUTPUT [BACKEND [CHECKPOINT]]")
        println("MODE = standard | weak | subcycling. NY = 2*NX. All units SI.")
        println("A short run is only a smoke test, not paper validation.")
        return
    end
    length(args) in (5,6,7) || error("Expected five arguments and optional backend/checkpoint; use --help")
    ...
    c=Config(nx=nx,ny=2nx,pressure_backend=backend)
    run_bubble(c;mode=mode,factor=factor,t_end=tend,output=output,
               snapshot_interval=max(tend/20,1e-12),resume=length(args)==7 ? args[7] : nothing)
end
