...
function main(args)
    ...
    cp(dirname(pathof(W)),joinpath(output,"src"))
    rows = Any[]
    ...
    grids=length(args)>=2 ? parse.(Int,split(args[2],',')) : [64,128,256]
    tend=length(args)>=3 ? parse(Float64,args[3]) : 0.001
    factor=length(args)>=4 ? parse(Float64,args[4]) : 1.0
    rtol=length(args)>=5 ? parse(Float64,args[5]) : 1e-9
    backend=length(args)>=6 ? Symbol(args[6]) : :jacobi
    for nx in grids
        c=Config(nx=nx,ny=2nx,gravity=0.0,poisson_rtol=rtol,pressure_backend=backend)
        ...
        open(joinpath(output,"history-$nx.csv"),"w") do io
            ...
            while s.t < tend
                step!(s,min(factor*stable_dt(s,c),tend-s.t),c;mode=:standard)
                d=diagnostics(s,c)
        ...
        row=Dict("nx"=>nx,"steps"=>s.steps,"elapsed_seconds"=>elapsed,
                 "peak_face_speed_bound"=>peak,"source_sha256"=>digest)
        ...
        open(joinpath(output,"summary.toml"),"w") do io
            TOML.print(io,Dict("status"=>"measured_requires_review","runs"=>rows,
                "julia_version"=>string(VERSION),"t_end"=>tend,
                "dt_factor"=>factor,"poisson_rtol"=>rtol,
                "pressure_backend"=>string(backend),
